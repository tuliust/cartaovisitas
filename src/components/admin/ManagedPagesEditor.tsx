...
type ManagedPagesEditorProps = { pageKey?: ManagedPageKey; showPreview?: boolean; showTabs?: boolean }

export function ManagedPagesEditor({ pageKey, showPreview = true, showTabs = true }: ManagedPagesEditorProps) {
  ...
  const selectedKey = pageKey ?? active
  ...
  return <div className="managed-pages-editor">
    ...
    {showTabs ? <div className="managed-page-tabs" role="tablist" aria-label="Páginas institucionais">{(Object.keys(labels) as ManagedPageKey[]).map((key) => <button type="button" role="tab" aria-selected={active === key} className={active === key ? 'active' : ''} key={key} onClick={() => setActive(key)}>{labels[key]}</button>)}</div> : null}
    <div className={`managed-page-layout${showPreview ? '' : ' managed-page-layout--no-preview'}`}><section className="brand-settings-section managed-page-form" aria-label={`Editar ${labels[selectedKey]}`}>
      ...
      {selectedKey === 'terms_and_privacy' ? <label>Versão e data textual<input value={page.version_label} maxLength={240} onChange={(event) => update({ ...page, version_label: event.target.value })} /></label> : null}
