...
function AnalyticsDashboard({ card }: { card: AdminBusinessCard }) {
  ...
  return <>
    ...
    {invalidRange ? <p className="admin-error" role="alert">Informe um período válido.</p> : loading ? <div className="state-card" role="status">Carregando estatísticas...</div> : error ? <p className="admin-error" role="alert">{error}</p> : <>
      <section className="analytics-kpis" aria-label="Indicadores principais"><article><span>Visualizações</span><strong>{counts.view}</strong></article><article><span>Interações com vCard</span><strong>{counts.vcard}</strong></article><article><span>Compartilhamentos</span><strong>{counts.share}</strong></article><article><span>Interações com QR Code</span><strong>{counts.qr}</strong></article><article><span>Última visualização</span><strong>{lastView ? new Date(lastView).toLocaleString('pt-BR') : 'Nenhuma'}</strong></article><article><span>Conversão para vCard</span><strong>{percent(counts.view ? counts.vcard / counts.view * 100 : 0)}</strong></article><article><span>Taxa de compartilhamento</span><strong>{percent(counts.view ? counts.share / counts.view * 100 : 0)}</strong></article></section>
