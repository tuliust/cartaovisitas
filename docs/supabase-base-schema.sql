...
-- Conteúdo editorial genérico; policies são aplicadas pela migration complementar.
create table if not exists public.managed_pages (
  id uuid primary key default gen_random_uuid(), page_key text not null unique, route_path text not null unique,
  title text not null, subtitle text, content jsonb not null, visibility text not null check (visibility in ('public','authenticated')),
  is_published boolean not null default true, version_label text, updated_by uuid references auth.users(id) on delete set null,
